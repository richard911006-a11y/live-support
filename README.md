...
7. 在命令输出中找到 Supergroup 的 `TELEGRAM_CHAT_ID=-100...`，并复制该值。

本项目 Worker 当前使用 `TELEGRAM_ADMIN_CHAT_IDS` 保存管理员 Chat ID；请将上一步得到的 ID 写入该变量。需要多个管理员或群组时，用英文逗号分隔多个 ID。`telegram:doctor` 只读取 Telegram API，不会修改 Bot、Webhook 或群组配置。
